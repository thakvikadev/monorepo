...
export const fromBase64 = (base64Str: string) => Buffer.from(base64Str, 'base64').toString();

// Convert a Base64URL string to ACSII string
export const fromBase64Url = (base64UrlStr: string) => Buffer.from(base64UrlStr, 'base64url').toString();
...
export const baseString = ({
  ...
  query: any;
  nonce: string;
}) => `${method?.toUpperCase()}&${encodeURIComponent(url)}&${bodyString(body, query)}&${encodeURIComponent(nonce)}`;
...
export const sign = (verifiableData: string, privateKey: string, passphrase?: string) =>
  crypto
...
export const verified = (verifiableData: string, publicKey: string, signature: string) => {
